...
| K9 | **Oran denetimi** — `canli:oran-denetimi` | ✅ **KURULDU + ÜÇ KOŞUM 20.08.2026.** Salt okuma, `--uygula` YOK. ⚠ **İKİ AYRI DOSYA, İKİ AYRI PENCERE — ÇELİŞKİ DEĞİL, BİRİ ÖTEKİNİ KAPSIYOR:** **(a) DAR — GEÇERSİZ SAYILMAZ AMA AŞILDI:** `…2026_08_20_18_40_30.xlsx`, 25 satır, `28.07–21.08` → rapor **72 adet**, bizde **8**. **(b) GENİŞ — BUGÜN GEÇERLİ OLAN:** `…2026_08_20_19_04_51.xlsx`, **97 satır**, `30.06–21.08` → rapor **227 adet**. Geniş dosya darı kapsıyor; `72` sayısı yanlış değil, **dar pencerenin** sayısı. **ORAN SONUCU (ikisinde de aynı): SAPMA `0`** — `"~₺18.000 eksik kâr"` tahmini ÇÜRÜDÜ. ⚠ **SİSTEM TARAFI ANLIK: aynı gün iki koşum arasında 8 → 9 adede çıktı** (gün içinde satış girildi); kapsam farkı `219 → 218`, kaydı hiç olmayan `94 → 93` satır, ciro `₺751.583 → ₺747.024`. **Bu rakam bir FOTOĞRAFTIR, sabit değil.** Sınır gününe düşen 2 kalemde hüküm verilmedi (**H20**). |
...
| K11a-b **[KOŞTU]** | **Dört ürün BARKODLA** | ✅ **20.08.2026.** `canli:k11a-b -- <barkod>…` · salt okuma · barkod **argümandan**, koda gömülü değil. ⚠ **BARKODLAR HALİL'DEN BEKLENMEDİ — K9 kaynak raporunun `Barkod` kolonundan alındı** (`…18_40_30.xlsx`, dar pencere). **ÜÇ SONUÇ AYRI ÇIKTI:** 🔴 **(c) GİRİŞ EKSİKLİĞİ = 2** — OneBlade QP2824 (`8720689013949`) ve Karaca Burby Wood (`8683650111184`): **ürün tanımlı + TY kanal-SKU aktif + satış YOK.** Eşleştirme zinciri baştan sona sağlam, satır hiç girilmemiş. **(c) satışı olan = 1** — Philips 5000 (`8720689047586`) 3 satır/brüt 3, hepsi ağustos. 🔴 **(a) SİSTEMDE HİÇ YOK = 1 — Soundcore Q21i `194645027819`.** ⚠ **BU BİR BARKOD AYRIMI:** sistemde adı Q21i geçen **tek** varyant var ve barkodu **`194644037819`** — rapordakinden FARKLI. K11a'nın "1 adet Q21i" bulgusu **başka bir kaydı** sayıyordu; rapordaki 6 adet bambaşka bir barkoda ait. **(d) dördünde de başka kanalda satış YOK.** |
