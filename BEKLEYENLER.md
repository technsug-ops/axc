...
| **K20** | 🟢 **HALİL YAPACAK — LEGO stok +1** | **Raf `A19` · barkod `5702017866932` · SKU `OYU-LG-598P-01`** (LEGO "Yukarı Bak" 43217). Ekran **3** gösteriyor, doğrusu **4**. İki hesap doğruluyor: FIFO açık partileri 4 · aritmetik `alınan 5 − satılan 1 = 4` (düzeltmelerin tamamı birbirini götürüyor). **Stok düzeltme → +1, birim maliyet `3599` GİRİLEREK** (boş bırakılırsa yeni maliyetsiz parti doğar, aynı hataya düşülür). Açıklama: _"iptal geri alma 1 fazla düşürdü (K20)"_. **Sayım isteğe bağlı**, zorunlu değil. |
| **K21** | 🔧 **BETİK GEREKİR — LEGO maliyet damgası** | `11518018178` NET-2 **3.188,75**, doğrusu ~**190**. Maliyet sistemde var (`ALM-NON-260813-02` · `297577854427` · **3.599**) ama **bağ kopuk**: 06:18'deki `ADJUSTMENT +2` maliyetsiz doğdu, FIFO onu tüketti, `SALE_OUT.unitCostAmount = null` kaldı. ⚠ **`canli:maliyet-hizala` BU VAKADA İŞE YARAMAZ** — damgayı partinin maliyetiyle hizalıyor, o partinin de maliyeti yok. Gereken: önce partiye `3599` damgası, sonra hizalama. **Ekrandan yapılamaz.** |
| **K22** | 🔧 **BETİK GEREKİR — Anker FIFO hayaleti** | ⚠ **TERS YÖNDE, DİKKAT: burada LEDGER DOĞRU (2), FIFO ŞİŞİK (3).** Raf `A3 — OFİS` · barkod `194644126629` · SKU `axcali1667`. **STOK DÜZELTMESİ YAPILMAZ** — ekrandaki 2 doğru (alınan 2, satılan 1, iptal edildi). Sorun: ayna hareket `sourceMovementId` taşıyor (17.08 öncesi davranış), hem partinin tüketimini sıfırlıyor hem kendisi parti sayılıyor. **Onarım: o tek alanın `null` yapılması** — miktara/paraya dokunmaz, iz bırakır. Sonra `canli:defter-ayrismasi` yeşile döner. |
