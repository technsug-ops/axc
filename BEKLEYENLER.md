...
| **K20** | 🔴 **STOK 1 EKSİK — düzeltme bekliyor** | **CANLI HATA 20.08.2026, kodu düzeltildi ama VERİ duruyor.** `11518018178`'in iptali geri alınırken stoktan **2 adet** düştü (satış 1 adetlik) — ayna süzgeci sonraki iptalin aynasını da tüketti. `OYU-LG-598P-01` (LEGO "Yukarı Bak") **1 adet eksik** görünüyor. ⚠ **Önce FİZİKSEL SAYIM:** bu ürünün hareket geçmişi çok karışık (mükerrer kayıt · HATA DÜZELTME ×2 · COUNT_CORRECTION). Ledger `3` diyor; rafta kaç tane olduğu söylenince stok düzeltme ekranından hizalanır. |
| **K21** | 🔴 **MALİYETSİZ SATIŞ — NET-2 ~₺3.000 şişik** | `11518018178` NET-2 **3.188,75** gösteriyor, doğrusu ~**190**. Sebep düzeltme değil: stok **maliyetsiz `ADJUSTMENT`** ile girilmiş (19.08 06:18, not: _"297577854427 sipariş no ile alınan bu ürüne ait ürünler"_) ve FIFO o partiyi tüketti → `SALE_OUT.unitCostAmount = null`. **Motor bugün yeniden hesaplasa da aynı rakamı veriyor** — bayat değil, veri eksik. **Gerekli:** `297577854427` numaralı alımın birim maliyeti. Girilince NET kendiliğinden doğruya döner. ⚠ Bu satış şu an **ciroya ve kâra bu şişik rakamla** giriyor. |
