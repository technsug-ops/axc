...
| # | İş | Durum |
|---|---|---|
| **K108** | **MUHASEBE DÖNEMİ KAVRAMI · [AÇIK — KURU KOŞUM YAPILDI, KOD YAZILMADI]** | 🕓 **[ÖLÇÜLDÜ 31.08.2026]** K107'nin **ön şartı** ama kendi başına da değerli: ay kapanışı, beyan dönemi, muhasebeciye verilecek tek sayfa. ⛔ **DÖNEM KAVRAMI SİSTEMDE HİÇ YOK — ölçüldü.** Şemada `periodStart`/`periodEnd` **yalnız `Settlement`** modelinde ve o **pazaryerinin ÖDEME dönemi**, muhasebe dönemi DEĞİL. İkisi karıştırılmamalı: hakediş dönemi kanalın takvimine, muhasebe dönemi bizim beyanımıza bağlı. 📏 **⭐ EN ÖNEMLİ ÖLÇÜM — KAPANMIŞ DÖNEME YAZIM NE SIKLIKTA:** ham rakam **10.061/10.774 (%93,4)** çıktı ve **bu rakam çerçevesiz KULLANILAMAZ**: bütün geçmiş 26–28 ağustosta bir kerede içe aktarıldı. Yığın günleri ayrıldı (tek günde ≥200 hareket → **10.232 hareket, %95,0**) ve geriye **günlük operasyonun 542 hareketi** kaldı: **212'si (%39,1) önceki aya yazılmış** — ve **204'ü `PURCHASE_IN`, yalnız 8'i `SALE_OUT`** (%96,2 geç girilen ALIM). ⭐ **BU ÖLÇÜM İKİ ŞEYİ BİRDEN SÖYLÜYOR:** ① tam yasak **kesinlikle yanlış** olurdu — günlük işin %39'unu kilitlerdi; ② Halil'in düzeltmesi (_"ciddi bir uyarı olsun, kesin aşılmaz kural değil"_) **veriyle doğrulandı**. _(29.08 sayım korumasında da aynı sonuç çıkmıştı: geriye dönük 15 hareketin 15'i de `PURCHASE_IN`.)_ ⭐ **TASARIM: KAPI DEĞİL DURAKSAMA — VE İKİNCİ BİR MANTIK YAZILMAZ.** `lib/sayim-korumasi.ts` deseni **AYNEN** yeniden kullanılır: saf karar gövdesi (`SERBEST` \| `DURAKSA`) · `israrGecerliMi()` (onay HER SEFERİNDE · sebep **kapalı listeden** · `DIGER` seçilirse açıklama zorunlu) · aynı ısrar ekranı · aynı iz yapısı. Sistem **DURUR ve sebebi yazar**: _"Bu dönem kapandı / bu yöntem değişikliği geçmiş kârı etkiler. Beyan edilmiş vergi tutmayabilir. Muhasebecinize danışın."_ **Israr edilirse GEÇER.** ⛔ **Yasak yok — karar firmanın, sorumluluk firmanın, kayıt sistemin.** 📐 **ŞEMA ÖNERİSİ (merdiven inildi):** ① mevcut alan: `Settlement.periodStart/End` **başka bir şeyin dönemi** ✗ · ② serbest metin / `AuditLog`: dönem durumu **her yazımda** sorgulanacak (`ORDER BY createdAt DESC LIMIT 1` her harekette) ve `AuditLog` "geriye bakmak" içindir ✗ · ③ türetilebilir mi: **hayır — kapanış bir KARARDIR**, veriden çıkmaz ✗ · ④ → **TABLO** (sütun değil, çünkü her dönem bir satır ve zamanla büyür): `MuhasebeDonemi { yil · ay · durum(ACIK\|KAPALI) · kapatanId · kapatildiAt · not }` + `@@unique([yil, ay])`. İstisna izi **iki yere**: `AuditLog` **ve** ilgili kayda _"uyarıya rağmen"_ damgası — ekranda **hangi dönem · hangi yöntem · kim · ne zaman · uyarıya rağmen mi** görünür. ⏭ **AÇIK KALAN — dönem raporu:** o ayın cirosu · kesintileri · NET-1/NET-2 · envanter değeri. **Verinin tamamı ZATEN var** (panel, rapor ve envanter ekranları aynı gövdelerden besleniyor); iş yeni hesap değil, **tek sayfada toplamak**. ⛔ **KOD YAZILMADI, MIGRATION KOŞULMADI** — şema onayı bekliyor. |
