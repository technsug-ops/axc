...
| # | İş | Durum |
|---|---|---|
| **K108** | **MUHASEBE DÖNEMİ · [YARIM — ŞEMA CANLIDA, KAPI HİÇBİR YOLA BAĞLI DEĞİL]** | ⛔ **BORÇ KAYDI 31.08.2026 — KULLANICI KARARI: DURDURULDU, yarın taze oturumda tek seferde.** ✅ **CANLIDA OLAN:** migration koştu (`20260831090000_muhasebe_donemi` · 42 migration · `MuhasebeDonemi` **0 satır** · StockMovement 10774 · Sale 5880 · ProductVariant 1104 **değişmedi** · `deploy:bekci` yeşil) · saf kural `lib/donem-korumasi.ts` · veri katmanı `lib/muhasebe-donemi.ts` · yedek listesine eklendi. ⛔ **KAPI HİÇBİR YOLA BAĞLI DEĞİL. Beş yol bekliyor:** satış · mal kabul · iade · içe aktarma · stok düzeltme. ⛔⛔ **VE BAĞLANANA KADAR DÖNEM KAPATMA EKRANI AÇILMAZ** — kullanıcı şartı: _"kapatılan dönem korunmaz, yanlış güvence verir."_ ⚠ **SIRA TERSİNE ÇEVRİLEMEZ:** ekran kapıdan ÖNCE yazılırsa Halil dönemi kapatır ve **korunduğunu sanır**; kapalı döneme yazım sessizce geçer. Önce **kapı**, sonra **kapatma düğmesi**. _(Bu, "yarım bağlanmış kapı, olmayan kapıdan tehlikelidir" — ekran korumalı görünür, bir yol açık kalır.)_ 📏 **KAPININ ÖLÇÜLMÜŞ TABANI (canlı, salt okuma):** günlük operasyonun **542 hareketinin 212'si (%39,1)** önceki aya yazılıyor ve **204'ü `PURCHASE_IN`** (%96,2 geç girilen alım) — tam yasak günlük işin %39'unu kilitlerdi, bu yüzden **duraksama**. ⏭ **YARIN — TEK TESLİM:** ① kapı 5 yola · ② ısrar ekranı (somut sayı: _"Bu dönemde N satış hesaplandı"_) · ③ dönem listesi + kapatma · ④ dönem raporu (mevcut gövdelerden, PDF) · ⑤ panel göstergesi · ⑥ bekçi + mutasyon (kapıyı atlayan · ısrar izini silen · raporu sayfalanmış kümeden toplayan · `Settlement.period*` ile karıştıran · **açık dönem yokken kilitlenen**). |
