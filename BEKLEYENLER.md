...
| **K116** | **SAĞLIK TURU KALICILAŞTIRMA — [AÇIK · K115② SONRASI]** | Kullanıcı kararı 31.08.2026. ① **BEKÇİ `sabit-gider:dogrula`** — `SABIT_GIDER`/`HIZMET_BEDELI`/`ODEME_GIDERI` satış başına **en fazla 1** kayıt; `canli-rakam-saglik` ölçümünden türer, bekçi turuna girer. Mutasyon: bölünmüş satışta ikinci sabit gider yazan kurgu **kırmızı yanmalı**. ② **ADET=0 KALEMİ — TEŞHİS (salt okuma)**: hangi satış · kaynağı (import/elle) · iptalle ilişkisi · NET'e etkisi; tek paragraf. ⛔ **sil/düzelt kararı raporda DEĞİL** — mimar + Halil'de. ③ **ENVANTER DEĞERİ EKRANI**: maliyeti `null` olan stok (bugün 14 adet) toplam değere **0 olarak karışmasın**; ayrı satır — _"maliyeti bilinmiyor: N adet"_. Kanıt şeridi ilkesi: bilinmeyen GÖRÜNÜR, uydurulmaz. ④ **PANO SATIRI** (aşağıya yazıldı). |
