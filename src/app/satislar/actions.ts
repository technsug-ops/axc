...
export type SiparisOnaySonucu =
  ...
  | Extract<OnayCekirdekSonucu, { tamam: false }>;
...
export async function siparisiOnayla(
  ...
  await yetkiIste("satis.yaz");
  try {
    /** ELLE VE OTOMATİK ONAY AYNI ÇEKİRDEKTEN (K168) — iki yerde iki
     *  ölçüt olmaz. Kabuk yalnız yetki + revalidate + kâr tazeleme. */
    const sonuc = await prisma.$transaction((tx) =>
      onayCekirdegi(tx, { saleId, secimler, otomatik: false }),
    );
    if (!sonuc.tamam) return sonuc;

    /** Kâr tazeleme İŞLEM DIŞINDA: yazılmış SALE_OUT'u okur (commit sonrası). */
